            ...
            for idir = 1:length(MuseStruct_micro{ipart})
                if isfield(MuseStruct_micro{ipart}{idir},'markers')
                    if isfield(MuseStruct_micro{ipart}{idir}.markers,(cfg.muse.startend{imarker,1}))
                        if ~isempty(MuseStruct_micro{ipart}{idir}.markers.(cfg.muse.startend{imarker,1}).events)
                            ...
                            for ifile = macro_filenrs
                                ...
                                for ievent = 1 : size(MuseStruct_macro{ipart}{idir}.markers.(cfg.muse.startend{imarker,1}).events,2)
                                    Startsample(ievent) = MuseStruct_macro{ipart}{idir}.markers.(cfg.muse.startend{imarker,1}).offset(ievent) + cfg.epoch.toi{imarker}(1) * hdr_macro.Fs - cfg.epoch.pad(imarker) * hdr_macro.Fs;
                                    Endsample(ievent)   = MuseStruct_macro{ipart}{idir}.markers.(cfg.muse.startend{imarker,2}).offset(ievent) + cfg.epoch.toi{imarker}(2) * hdr_macro.Fs + cfg.epoch.pad(imarker) * hdr_macro.Fs;
                                    trialnr(ievent)     = ievent;
                                    Stage(ievent)       = -1;
