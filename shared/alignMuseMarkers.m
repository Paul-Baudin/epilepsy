    ...
    for ipart = 1 : size(cfg.directorylist,2)
        ...
        for imarker = markerlist
            ...
            for idir = markerindx
                ...
                cfgtemp             = [];
                if strcmp(channeltype,'micro')
                    cfgtemp.trl         = [MuseStruct_micro{ipart}{idir}.markers.(cfg.muse.startend{imarker,1}).offset'+cfg.align.toibaseline{imarker}(1)*dat_filt.fsample, ...
                        MuseStruct_micro{ipart}{idir}.markers.(cfg.muse.startend{imarker,1}).offset'+cfg.align.toiactive{imarker}(2)*dat_filt.fsample];
                else
                    cfgtemp.trl         = [MuseStruct_macro{ipart}{idir}.markers.(cfg.muse.startend{imarker,1}).offset'+cfg.align.toibaseline{imarker}(1)*dat_filt.fsample, ...
                        MuseStruct_macro{ipart}{idir}.markers.(cfg.muse.startend{imarker,1}).offset'+cfg.align.toiactive{imarker}(2)*dat_filt.fsample];
                end
...
%                 MuseStruct_micro{ipart}{idir}.markers.(cfg.muse.startend{imarker,1}).events    = MuseStruct_micro{ipart}{idir}.markers.(cfg.muse.startend{imarker,1}).events(~hasartefact & haspeak);
